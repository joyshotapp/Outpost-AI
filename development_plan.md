...
### Sprint 2（Week 3-4）：供應商 CRUD + 檔案上傳 + 基礎 UI
...
| 2.12 | 供應商註冊 + 資料填寫的 E2E 測試 | QA-1 | 2 | 2.4, 2.6 | Playwright 自動化測試通過 |
...
### Sprint 3（Week 5-6）：RFQ 解析 + Lead Scoring（亮點三 + 亮點六）
...
| 3.13 | RFQ 解析準確度測試（用 20 筆不同類型的真實 RFQ） | QA-1 + AI-1 | 3 | 3.8 | 結構化輸出準確率 ≥ 80%，Lead Score 分級正確率 ≥ 85% |
...
### Sprint 4（Week 7-8）：AI 數位業務分身（亮點四）
...
| 4.8 | AI 分身聊天 Widget UI（嵌入式浮動視窗） | FE-1 | 5 | 4.6 | 供應商頁面右下角浮動聊天視窗，多語言切換，打字動畫 |
...
### Sprint 5（Week 9-10）：訪客意圖分析儀表板（亮點二）
...
| 5.5 | 訪客意圖評分引擎（行為分 + 企業分 → 合併） | AI-1 | 3 | 5.1, 5.2, 5.3 | 行為評分 + 企業評分 → 合併分數，閾值觸發通知 |
...
### Sprint 6（Week 11-12）：AI 多語系影片生成（亮點一）
...
| 6.3 | 德語字數壓縮處理（翻譯後自動檢查字數比，超過閾值自動壓縮） | AI-1 | 3 | 6.2 | 德語版本影片時長不超過原始版本 110% |
...
| 6.7 | HeyGen 費用追蹤（每月用量監控 + 超量警告） | INT-1 + OPS-1 | 2 | 6.1 | Dashboard 顯示月度用量，超過閾值自動告警 |
...
### Sprint 7（Week 13-14）：Outbound 引擎 — 名單建立與 LinkedIn 外展
...
### Sprint 8（Week 15-16）：Outbound 引擎 — Email 外展 + 統一處理
...
### Sprint 9（Week 17-18）：內容裂變矩陣（亮點五）
...
### Sprint 10（Week 19-20）：搜尋系統 + 買家前台完善
...
| 10.4 | 供應商列表頁 UI（搜尋結果 + 篩選側邊欄 + 卡片/列表切換） | FE-1 | 4 | 10.2 | 篩選器流暢、分頁載入、結果排序 |
...
### Sprint 11（Week 21-22）：訂閱計費 + 管理後台
...
### Sprint 12（Week 23-24）：進階功能 + 分析儀表板

> 目標：供應商 KPI 儀表板、展覽整合、名片掃描、再行銷序列

| # | Task | 負責 | 天數 | 前置 | 驗收標準 |
|---|------|------|------|------|---------|
| 12.1 | 供應商 KPI 儀表板（線上系統指標 + 業務成交指標） | FE-2 + BE-1 | 5 | 全部前置完成 | Recharts 圖表：RFQ 趨勢、Lead Score 分佈、訪客趨勢、Outbound 成效、內容觸及率 |
| 12.2 | 展覽活動管理模組（展前 ICP 名單 + 展中名片掃描 + 展後序列） | BE-1 + FE-2 | 5 | 8.5 | 支援展覽生命週期管理 |
| 12.3 | 名片掃描 OCR（手機拍照 → Claude Vision → 結構化聯絡人） | AI-1 + FE-1 | 3 | 3.2 | 拍照名片 → 自動建立 CRM 聯絡人 |
...
### Sprint 13（Week 25-26）：整合測試 + 效能優化
...
| 13.9 | 安全掃描（OWASP ZAP + 依賴漏洞掃描 + API 安全測試） | OPS-1 + QA-1 | 3 | All | 無 High/Critical 漏洞 |
| 13.10 | 第三方 API 降級測試（模擬各 API 掛掉的 Fallback 行為） | QA-1 + INT-1 | 2 | All | 每個 API 有明確的降級策略 |
...
### Sprint 14（Week 27-28）：上線準備 + 正式發布
