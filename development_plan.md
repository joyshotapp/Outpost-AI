...
### Sprint 11（Week 21-22）：訂閱計費 + 管理後台 — � 工程關版
...
**Sprint 11 交付物**：
- Stripe 訂閱計費系統（stub 模式 + 正式模式，含 Webhook）
- 功能門禁（free/starter/professional/enterprise 四層）
- 供應商訂閱管理頁面（方案卡片、帳單歷史、功能門禁視覺化）
- 管理後台完整 7 大頁面（KPI / 供應商 / 買家 / 內容 / Outbound / API 用量 / 設定）
- 12 個新 Admin API 端點 + 8 個 Billing API 端點
- Migration 012：subscriptions / api_usage_records / system_settings 三張新表
- E2E 測試：`tests/test_sprint11_e2e.py`（50+ 測試案例）

**Sprint 11 完成時間**：Week 21-22 ✅
